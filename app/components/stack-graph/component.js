...
export default Ember.Component.extend(ThrottledResize, {
  ...
  showService: function(id) {
    let svgHeight;
    if ( id ) {
        var serviceInstance = this.get('model.stack.services').findBy('id', id);

        if ( !serviceInstance ) {
            serviceInstance = this.get('crosslinkServices').findBy('id', id);
        }
        ...
        if ( serviceInstance ) {
            ...
            this.set('model.selectedService', serviceInstance);
        }
  ...
  updateGraph: function() {
    ...
    expectedServices.forEach((service) => {
      ...
      html = `<span data-service="${service.get('id')}"></span><i class="icon  ${activeIcon(service)}"></i>
                  <h4 class="clip">${stackName}${Util.escapeHtml(service.get('displayName'))}</h4>
